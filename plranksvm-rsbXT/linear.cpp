...
class selection_rank_fun: public function
{
	...
	protected:
		...
		blas_sparse_matrix spr_X;//please notice it.
		blas_sparse_matrix spr_XT;
...
selection_rank_fun::selection_rank_fun(const problem *prob, double C, int thread_count, int nr_subset, int *perm, int *start, int *count)
{
	...
	//BLAS sparse matrix added for L2-regularized ranksvm.	
	int *row_index, *col_index;
	double *values;
	row_index = new int[prob->nnz];
	col_index = new int[prob->nnz];
	values = new double[prob->nnz];
	now = 0;
	for(i = 0; i < l ; i++)
	{
		...
		{
			row_index[now] = i;
			col_index[now] = (s->index)-1;
			values[now++] = s->value;
			s++;
		}
	}
	//library initialization.		
	spr_X = blas_invalid_handle;
	spr_XT = blas_invalid_handle;
	spr_X = BLAS_duscr_begin(prob->l, prob->n);
	spr_XT = BLAS_duscr_begin(prob->n, prob->l);
	BLAS_duscr_insert_entries(spr_X, prob->nnz, values, row_index, col_index);
	BLAS_duscr_insert_entries(spr_XT, prob->nnz, values, col_index, row_index); 
	BLAS_duscr_end(spr_X);
	BLAS_duscr_end(spr_XT);
	delete[] row_index;
	delete[] col_index;
	delete[] values;
...
void selection_rank_fun::Xv(double *v, double *Xv)
{
	...
	for(i = 0; i < l; i++)
		Xv[i] = 0.0;
	if(BLAS_dusmv(blas_no_trans, 1, spr_X, v, 1, Xv, 1))
		printf("dusmv Xv error\n");
}
